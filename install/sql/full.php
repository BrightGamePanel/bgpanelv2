...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
	...
	*/
	...
	INSERT INTO ".DB_PREFIX."game (
	  game_id, name,								status,		max_slots, default_port, query_port, query_type, config,						launcher,
	  			cache_dir
	)
	VALUES
	  ('1', 'Counter-Strike: Source',				'Active',	'16',  '27015', '27015',	'source',			'".serialize($GAME_DB[1])."',	'./srcds_run -game cstrike -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -nohltv -autoupdate',
	  			'~/game-repositories/css/'),
	  ('2', 'Day of Defeat: Source',				'Active',	'16',  '27015', '27015',	'source',			'".serialize($GAME_DB[2])."',	'./srcds_run -game dod -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -tickrate {cfg2} -nohltv -autoupdate',
	  			'~/game-repositories/dods/'),
	  ('3', 'Half-Life 2: Deathmatch',				'Active',	'16',  '27015', '27015',	'source',			'".serialize($GAME_DB[3])."',	'./srcds_run -game hl2mp -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -tickrate {cfg2} -nohltv -autoupdate',
	  			'~/game-repositories/hl2dm/'),
	  ('4', 'Team Fortress 2',						'Active',	'24',  '27015', '27015',	'source',			'".serialize($GAME_DB[4])."',	'./srcds_run -game tf -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -nohltv -autoupdate',
	  			'~/game-repositories/tf2/'),
	  ('5', 'Left 4 Dead',							'Active',	'8',   '27015', '27015',	'source',			'".serialize($GAME_DB[5])."',	'./srcds_run -game left4dead -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -nohltv -autoupdate',
	  			'~/game-repositories/left4dead/'),
	  ('6', 'Left 4 Dead 2',						'Active',	'8',   '27015', '27015',	'source',			'".serialize($GAME_DB[6])."',	'./srcds_run -game left4dead2 -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -nohltv -autoupdate',
	  			'~/game-repositories/left4dead2/'),
	  ('7', 'Counter-Strike',						'Active',	'16',  '27015', '27015',	'halflife', 		'".serialize($GAME_DB[7])."',	'./hlds_run -game cstrike +ip {ip} +port {port} +maxplayers {slots} +map {cfg1} -pingboost {cfg2} -autoupdate',
	  			'~/game-repositories/cstrike/'),
	  ('8', 'Killing Floor',						'Inactive', '6',   '7707', 	'7708',		'killingfloor',		'".serialize($GAME_DB[8])."',	'./ucc-bin server {cfg1}?game=KFmod.KFGameType?VACSecure={cfg2}?MaxPlayers={slots}?AdminName={cfg3}?AdminPassword={cfg4} -nohomedir ini={cfg5}',
	  			'~/game-repositories/kfserver/'),
	  ('9', 'Call of Duty 4: Modern Warfare',		'Inactive', '18',  '28960', '28960',	'callofduty4', 		'".serialize($GAME_DB[9])."',	'./cod4_lnxded +exec {cfg1} +set sv_maxclients {slots} +map_rotate +set net_ip {ip} +set net_port {port} +set fs_homepath {cfg2} +set fs_basepath {cfg3} +set dedicated 2',
	  			'~/game-repositories/cod4/'),
	  ('10', 'Minecraft',							'Active',	'24',  '25565', '25565',	'minecraft',		'".serialize($GAME_DB[10])."',	'java -Xms1024M -Xmx1024M -jar minecraft_server.jar nogui',
	  			'~/game-repositories/minecraft/'),
	  ('11', 'Call of Duty: Modern Warfare 3',		'Active',	'18',  '27015', '27016',	'callofdutymw3',	'".serialize($GAME_DB[11])."',	'xvfb-run -a wine iw5mp_server.exe +set sv_config {cfg4} +set sv_maxclients {slots} +start_map_rotate +set net_ip {ip} +set net_port {port} +set net_queryPort {cfg1} +set net_authPort {cfg2} +set net_masterServerPort {cfg3} +set dedicated 2',
	  			'~/game-repositories/codmw3/'),
	  ('12', 'Call of Duty 2',						'Inactive', '32',  '28960', '28960',	'callofduty2',		'".serialize($GAME_DB[12])."',	'./cod2_lnxded +exec {cfg1} +map_rotate +set net_ip {ip} +set net_port {port} +set fs_homepath {cfg2} +set fs_basepath {cfg3} +set dedicated 2',
	  			'~/game-repositories/cod2/'),
	  ('13', 'Call of Duty: World at War',			'Inactive', '32',  '28960', '28960',	'callofdutywaw', 	'".serialize($GAME_DB[13])."',	'./codwaw_lnxded +exec {cfg1} +set sv_maxclients {slots} +map_rotate +set net_ip {ip} +set net_port {port} +set fs_homepath {cfg2} +set fs_basepath {cfg3} +set dedicated 2',
	  			'~/game-repositories/codwaw/'),
	  ('14', 'Wolfenstein: Enemy Territory',		'Inactive', '32',  '27960', '27960',	'wolfet', 			'".serialize($GAME_DB[14])."',	'./etded +exec {cfg1} +sv_maxclients {slots} +set fs_homepath {cfg2} +set fs_basepath {cfg3} +set net_port {port}',
	  			'~/game-repositories/wolfet/'),
	  ('15', 'ArmA: 2',								'Active',	'64',  '2302', 	'2302',		'arma2',			'".serialize($GAME_DB[15])."',	'./server -config={cfg1} -netlog -port={port}',
	  			'~/game-repositories/arma2/'),
	  ('16', 'Garrysmod',							'Active',	'16',  '27015', '27015',	'source',			'".serialize($GAME_DB[16])."',	'./srcds_run -game garrysmod -ip {ip} -port {port} -maxplayers {slots} +map {cfg1} -nohltv -autoupdate',
	  			'~/game-repositories/garrysmod/'),
	  ('17', 'Counter-Strike: Global Offensive',	'Active',	'24',  '27015', '27015',	'source',			'".serialize($GAME_DB[17])."',	'./srcds_run -game csgo -console -usercon -secure -nohltv -tickrate {cfg5} +ip {ip} +hostport {port} -maxplayers_override {slots} +map {cfg1} +mapgroup {cfg2} +game_type {cfg3} +game_mode {cfg4}',
	  			'~/game-repositories/csgo/'),
	  ('18', 'ArmA: Armed Assault',					'Active',	'64',  '2302',  '2302',		'arma',				'".serialize($GAME_DB[18])."',	'./server -config={cfg1} -netlog -port={port}',
	  			'~/game-repositories/arma/'),
	  ('19', 'Battlefield 2',						'Active',	'64',  '16567', '29900',	'bf2',				'".serialize($GAME_DB[19])."',	'./start.sh',
	  			'~/game-repositories/bf2/'),
	  ('20', 'Battlefield 1942',					'Active',	'64',  '14567', '23000',	'bf1942', 			'".serialize($GAME_DB[20])."',	'./start.sh +statusMonitor 1',
	  			'~/game-repositories/bf1942/'),
	  ('21', 'Multi Theft Auto',					'Active',	'128', '22003', '22126',	'mta',				'".serialize($GAME_DB[21])."',	'./mta-server -t --ip {ip} --port {port} --httpport {port} --maxplayers {slots}',
	  			'~/game-repositories/mta/'),
	  ('22', 'San Andreas: Multiplayer (SA-MP)',	'Active',	'128', '7777',  '7777',		'samp',				'".serialize($GAME_DB[22])."',	'./samp03svr',
	  			'~/game-repositories/samp/'),
	  ('23', 'Urban Terror',						'Active',	'32',  '27960', '27960',	'urbanterror',		'".serialize($GAME_DB[23])."',	'./ioUrTded.i386 +set fs_game q3ut4 +set net_port {port} +set com_hunkmegs 128 +exec {cfg1} +set dedicated 2',
